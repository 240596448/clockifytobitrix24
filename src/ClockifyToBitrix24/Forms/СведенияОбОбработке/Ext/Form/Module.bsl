...
Процедура ПроверитьАктуальностьВерсии()
	...
	НомерВерсииНаГитхаб = Версия;
	Попытка
		НомерВерсииНаГитхаб = ОбработкаОбъект.GitHub_ПолучитьНомерПоследнейВерсии();
	Исключение
	КонецПопытки;
	НомерВерсииНаГитхаб = Число(СтрЗаменить(НомерВерсииНаГитхаб, ".", ""));
	НомерТекущейВерсии = Число(СтрЗаменить(Версия, ".", ""));
	
	Элементы.ВерсияСостояние.Видимость = НомерТекущейВерсии < НомерВерсииНаГитхаб;
