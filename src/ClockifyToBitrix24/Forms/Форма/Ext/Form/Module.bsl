...
Процедура ДатаСПриИзменении(Элемент)
	...
	Если ДатаС > ТекущаяДата() Тогда
		ДатаС = ТекущаяДата();
		ОчиститьСообщения();
		СообщитьПользователю("Нельзя выбрать дату больше текущего дня", Неопределено, "ДатаС");
	КонецЕсли;
...
Функция ПропуститьЗапись(Знач ДанныеОВремениКлокифай,
	...
	ДатаЗаписиМеньшеДатыОграничения = ЗначениеЗаполнено(ДатаС) И ДанныеОВремениКлокифай.ДатаНачала < ДатаС;
	Если ДатаЗаписиМеньшеДатыОграничения Тогда
		Возврат Истина;
	КонецЕсли;
	
	ДатаЗаписиБольшеДатыОграничения = ЗначениеЗаполнено(ДатаПо) И ДанныеОВремениКлокифай.ДатаНачала > КонецДня(ДатаПо);
