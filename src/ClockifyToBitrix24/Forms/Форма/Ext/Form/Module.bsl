...
Процедура ПриОткрытии(Отказ)
	...
	ЗатраченноеВремяПересчитатьПодвал();
	ОшибкиПересчитатьПодвал();
	УстановитьВидимостьОшибок();
	ЗатраченноеВремяСброситьОтборСтрок();
	ЗаполнитьБитриксСписокЗадач();
	ЗаполнитьБитриксСтатус();
...
&НаКлиенте
Процедура ФильтрПоСтатусуЗадачиПриИзменении(Элемент)
...
Процедура ДекорацияСведенияОбОбработкеНажатие(Элемент)
	...
	ФормыОбработки = Лев(ИмяФормы, СтрНайти(ИмяФормы, ".", , , 3)); 
	ОткрытьФорму(ФормыОбработки + "СведенияОбОбработке",,
		ЭтаФорма,,,,,
...
Процедура ПериодПриИзменении(Элемент)
	...
	Если Период.ДатаНачала > ТекущаяДата() Тогда
		Период.ДатаНачала = НачалоДня(ТекущаяДата());
		ОчиститьСообщения();
		СообщитьПользователю("Нельзя выбрать дату начала периода больше текущего дня", Неопределено, "Период");
	КонецЕсли;
...
// Возвращает структуру с ключами подобными колонкам таблицы ЗатраченноеВремя
//
// Параметры:
//  ClockifyДанные  - Структура - Данные записи о времени с сайта Clockyfy с ключами
//    * timeInterval - Структура - Коллекция с ключами start и end, хранит данные о времени
//    * description - Строка - Комментарий к записи о времени
//
// Возвращаемое значение:
//   Структура   - Структура с ключами
//    * Дата - Дата - Дата начала записи
//    * ДатаНачала - Дата - Дата начала записи
//    * ДатаОкончания - Дата - Дата окончания записи
//    * ЗатраченоСекунд - Число - Количество затраченных секунд
//    * ЗатраченоеВремя - Дата - Итого затраченного времени
//    * КомментарийКлокифай - Строка - Оригинальный комментарий Clockify
//    * Комментарий - Строка - Чистый комментарий очищенный от номера задачи Битрикс
//    * ТаймерОстановлен - Булево - Признак остановки таймера
//
&НаСервереБезКонтекста
Функция ПрочитатьПараметрыЗаписиClockify(ClockifyДанные)
	
	ПараметрыЗаписиОВремени = Новый Структура();
	
	ПараметрыЗаписиОВремени.Вставить("Дата");
	ПараметрыЗаписиОВремени.Вставить("ДатаНачала");
	ПараметрыЗаписиОВремени.Вставить("ДатаОкончания");
	ПараметрыЗаписиОВремени.Вставить("ЗатраченоСекунд");
	ПараметрыЗаписиОВремени.Вставить("ЗатраченоеВремя");
	ПараметрыЗаписиОВремени.Вставить("КомментарийКлокифай");
	ПараметрыЗаписиОВремени.Вставить("Комментарий");
	ПараметрыЗаписиОВремени.Вставить("ТаймерОстановлен", ЗначениеЗаполнено(ClockifyДанные.timeInterval.end));
	
	Если Не ЗначениеЗаполнено(ClockifyДанные) Тогда
		Возврат ПараметрыЗаписиОВремени;
	КонецЕсли;
	
	ПараметрыЗаписиОВремени.Дата = ПрочитатьДатуJSON(ClockifyДанные.timeInterval.start, ФорматДатыJSON.ISO);
	ПараметрыЗаписиОВремени.ДатаНачала = ПараметрыЗаписиОВремени.Дата;
	Если ПараметрыЗаписиОВремени.ТаймерОстановлен Тогда
		ПараметрыЗаписиОВремени.ДатаОкончания = ПрочитатьДатуJSON(ClockifyДанные.timeInterval.end, ФорматДатыJSON.ISO);
	Иначе
		ПараметрыЗаписиОВремени.ДатаОкончания = ТекущаяДатаСеанса();
	КонецЕсли;
	ПараметрыЗаписиОВремени.ЗатраченоСекунд = ПараметрыЗаписиОВремени.ДатаОкончания - ПараметрыЗаписиОВремени.ДатаНачала;
	ПараметрыЗаписиОВремени.ЗатраченоеВремя = Дата(1,1,1) + ПараметрыЗаписиОВремени.ЗатраченоСекунд;
	ПараметрыЗаписиОВремени.КомментарийКлокифай = ClockifyДанные.description;
	БЗНомер = СтрРазделить(ClockifyДанные.description, " ", Ложь);
	Если ЗначениеЗаполнено(БЗНомер) Тогда
		БЗНомер = СокрЛП(СтрРазделить(ClockifyДанные.description, " ", Ложь)[0]);
	КонецЕсли;
	
	Если ЭтоНомерЗадачи(БЗНомер) Тогда	
		ПараметрыЗаписиОВремени.Комментарий = СокрЛП(Прав(ClockifyДанные.description, СтрДлина(ClockifyДанные.description) - СтрДлина(БЗНомер)));
	Иначе
		ПараметрыЗаписиОВремени.Комментарий = ClockifyДанные.description;
	КонецЕсли;
...
Процедура ЗаполнитьТЗЗатраченноеВремя(ClockifyДанныеПоЗатраченномуВремени, БитриксЗатраченноВремя, МассивНомеровЗадач)
	...
	Для Каждого ClockifyДанные Из ClockifyДанныеПоЗатраченномуВремени Цикл
		...
		ДанныеОВремениКлокифай = ПрочитатьПараметрыЗаписиClockify(ClockifyДанные);
		
		Если ПропуститьЗапись(ДанныеОВремениКлокифай,
		...
		ЗаполнитьЗначенияСвойств(СтрЗатраченоеВремя, ДанныеОВремениКлокифай);
		
		ОтборВремяБитрикс = Новый Структура;
		ОтборВремяБитрикс.Вставить("ДатаСоздания", ДанныеОВремениКлокифай.ДатаНачала);
		ОтборВремяБитрикс.Вставить("Секунды", ДанныеОВремениКлокифай.ЗатраченоСекунд);
		...
		Если НЕ СтрИнформацияПоЗадачамБитрикс = Неопределено Тогда
			ЗаполнитьЗначенияСвойств(СтрЗатраченоеВремя, СтрИнформацияПоЗадачамБитрикс);
		КонецЕсли;
...
Функция ВремяДляБитрикс()
	...
	БитриксUTCСекунд = БитриксUTCСекунд();
	...
	Для Каждого СтрЗатраченноеВремя Из ЗатраченноеВремя Цикл
		...
		ДеталиОЗатраченномВремени.Вставить("CREATED_DATE", Формат(СтрЗатраченноеВремя.Дата + БитриксUTCСекунд, "ДФ='yyyy-MM-dd H:mm:ss'"));
...
&НаСервере
Функция БитриксUTCСекунд()
	
	БитриксUTCСекунд = 0;
	
	СекундВЧасе = 3600;
	БитриксUTCСекунд = BitrixUTC * СекундВЧасе;
	
	Возврат БитриксUTCСекунд;
	
КонецФункции // БитриксUTCСекунд()
...
//
&НаСервере
Процедура ДобавитьВОшибки(Знач ДанныеОВремениКлокифай, Знач ТекстОшибки)
	
	СтрОшибки = Ошибки.Добавить();
	
	ЗаполнитьЗначенияСвойств(СтрОшибки, ДанныеОВремениКлокифай);
...
//
&НаСервере
Функция ПропуститьЗапись(Знач ДанныеОВремениКлокифай,
						 МассивАктуальныхНомеровЗадач = Неопределено,
						 ДобавлятьВТаблицуОшибки = Ложь)
	
	ДатаЗаписиМеньшеДатыОграничения = ЗначениеЗаполнено(Период.ДатаНачала) И ДанныеОВремениКлокифай.ДатаНачала < Период.ДатаНачала;
	Если ДатаЗаписиМеньшеДатыОграничения Тогда
		Возврат Истина;
	КонецЕсли;
	
	ДатаЗаписиБольшеДатыОграничения = ЗначениеЗаполнено(Период.ДатаОкончания) И ДанныеОВремениКлокифай.ДатаНачала > КонецДня(Период.ДатаОкончания);
	Если ДатаЗаписиБольшеДатыОграничения Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если Не ДанныеОВремениКлокифай.ТаймерОстановлен Тогда
		
		Если ДобавлятьВТаблицуОшибки Тогда
			ТекстОшибки = "Остановите таймер по текущей записи.";
			ДобавитьВОшибки(ДанныеОВремениКлокифай, ТекстОшибки);
		КонецЕсли;
		
		Возврат Истина;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДанныеОВремениКлокифай.КомментарийКлокифай) Тогда
	...
	НомерЗадачиИОписаниеНЕРазделеныПробелом = СтрНайти(ДанныеОВремениКлокифай.КомментарийКлокифай, " ", НаправлениеПоиска.СНачала) = 0;
	...
	НомерЗадачи = СокрЛП(СтрРазделить(ДанныеОВремениКлокифай.КомментарийКлокифай, " ", Ложь)[0]);
	...
	Если ЗначениеЗаполнено(МассивАктуальныхНомеровЗадач) Тогда
		...
		Если МассивАктуальныхНомеровЗадач.Найти(НомерЗадачи) = Неопределено Тогда
			ТекстОшибки = СтрШаблон("Укажите верный номер задачи Битрикс24: задача ""%1"" не найдена или недостаточно прав на действия с ней.", НомерЗадачи);;
			ДобавитьВОшибки(ДанныеОВремениКлокифай, ТекстОшибки);
...
Функция НомераЗадачБитрикс(ClockifyДанныеПоЗатраченномуВремени)
	...
	Для каждого ClockifyДанные Из ClockifyДанныеПоЗатраченномуВремени Цикл
		...
		Если ПропуститьЗапись(ДанныеОВремениКлокифай) Тогда
			Продолжить;
		КонецЕсли;
...
&НаКлиенте
Процедура Bitrix24WebHookПриИзменении(Элемент)
	
	Если ПустаяСтрока(Bitrix24WebHook) Тогда
		Возврат;
	КонецЕсли;
	
	ПозРест = СтрНайти(Bitrix24WebHook, "rest/")+5;
	ИДПользователяБитрикс = Сред(Bitrix24WebHook, ПозРест, СтрНайти(Bitrix24WebHook,"/", ,ПозРест)-ПозРест); 
...
&НаКлиенте
Процедура ЗатраченноеВремяПересчитатьПодвал()
	...
	Для Каждого СтрЗатраченноеВремя Из ЗатраченноеВремя Цикл
		...
		ЗатраченоСекунд = СтрЗатраченноеВремя.ДатаОкончания - СтрЗатраченноеВремя.ДатаНачала;
		ИтогоЗатраченоСекунд = ИтогоЗатраченоСекунд + ЗатраченоСекунд;
...
&НаКлиенте
Процедура ОшибкиПересчитатьПодвал()
	
	Если Не ЗначениеЗаполнено(Ошибки) Тогда
		Возврат;
	КонецЕсли;
	
	Если Не Элементы.Ошибки.Видимость Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрОшибка Из Ошибки Цикл
		...
		ЗатраченоСекунд = СтрОшибка.ДатаОкончания - СтрОшибка.ДатаНачала;
		ИтогоЗатраченоСекунд = ИтогоЗатраченоСекунд + ЗатраченоСекунд;
	...
	Элементы.ОшибкиЗатраченоеВремя.ТекстПодвала = СтрШаблон("%1 ч. %2 м. %3 с.", ЗатраченныеЧасы, Минуты, Секунды);
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьВидимостьОшибок()
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Элементы.СтраницаОшибки.Заголовок = СтрШаблон("Ошибки (%1)", КоличествоОшибок);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьБитриксСтатус()
	
	Элементы.ФильтрПоСтатусуЗадачи.СписокВыбора.Очистить();
	
	Элементы.ФильтрПоСтатусуЗадачи.СписокВыбора.Добавить("", "Все");
	Элементы.ФильтрПоСтатусуЗадачи.СписокВыбора.Добавить("Истина", "Завершённые",);
	Элементы.ФильтрПоСтатусуЗадачи.СписокВыбора.Добавить("Ложь", "Не завершённые",);
	ФильтрПоСтатусуЗадачи = Элементы.ФильтрПоСтатусуЗадачи.СписокВыбора[0];
	
КонецПроцедуры

&НаКлиенте
Процедура ЗатраченноеВремяСброситьОтборСтрок()
...
Процедура ЗатраченноеВремяУстановитьОтборСтрок()
	...
	Если СтрНайти(ФильтрПоСтатусуЗадачи,"Все") = 0 И ФильтрПоСтатусуЗадачи <> "" Тогда
		СтруктураПоиска.Вставить("Завершена", Булево(ФильтрПоСтатусуЗадачи));
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовСтраницыВремя()
	...
	ДоступностьЭлементов = ЗначениеЗаполнено(ЗатраченноеВремя);
	
	Элементы.ГруппаФильтровЗатраченноеВремя.Доступность = ДоступностьЭлементов;
	Элементы.ЭкспортироватьВремяВБитрикс.Доступность = ДоступностьЭлементов;
